...
3. Stack starten:
   ...
   docker compose up -d
   ```
   Beim Start erzeugt der Mosquitto-Container (eine lokal gebaute Ableitung des offiziellen Images) automatisch eine Passwortdatei
   in `mosquitto/config/passwordfile` auf Basis der gesetzten Variablen `MQTT_USERNAME` und `MQTT_PASSWORD`.
